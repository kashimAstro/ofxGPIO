...
// Pointers to the hardware register bases
	...
	volatile uint32_t *bcm2835_gpio = static_cast<uint32_t *>(MAP_FAILED);
	volatile uint32_t *bcm2835_pwm  = static_cast<uint32_t *>(MAP_FAILED);
	volatile uint32_t *bcm2835_clk  = static_cast<uint32_t *>(MAP_FAILED);
	volatile uint32_t *bcm2835_pads = static_cast<uint32_t *>(MAP_FAILED);
	volatile uint32_t *bcm2835_spi0 = static_cast<uint32_t *>(MAP_FAILED);
	volatile uint32_t *bcm2835_bsc0 = static_cast<uint32_t *>(MAP_FAILED);
	volatile uint32_t *bcm2835_bsc1 = static_cast<uint32_t *>(MAP_FAILED);
	volatile uint32_t *bcm2835_st	= static_cast<uint32_t *>(MAP_FAILED);
#else
...
int bcm2835_init(void)
{
    ...
    #ifdef COMPILE_WITHOUT_OPENFRAMEWORKS
	    // GPIO:
	    bcm2835_gpio = static_cast<uint32_t *>( mapmem("gpio", BCM2835_BLOCK_SIZE, memfd, BCM2835_GPIO_BASE) );
	    if (bcm2835_gpio == MAP_FAILED) goto exit;

	    // PWM
	    bcm2835_pwm = static_cast<uint32_t *>( mapmem("pwm", BCM2835_BLOCK_SIZE, memfd, BCM2835_GPIO_PWM) );
	    if (bcm2835_pwm == MAP_FAILED) goto exit;

	    // Clock control (needed for PWM)
	    bcm2835_clk = static_cast<uint32_t *>( mapmem("clk", BCM2835_BLOCK_SIZE, memfd, BCM2835_CLOCK_BASE) );
	    if (bcm2835_clk == MAP_FAILED) goto exit;
	    
	    bcm2835_pads = static_cast<uint32_t *>( mapmem("pads", BCM2835_BLOCK_SIZE, memfd, BCM2835_GPIO_PADS) );
	    if (bcm2835_pads == MAP_FAILED) goto exit;
	    
	    bcm2835_spi0 = static_cast<uint32_t *>( mapmem("spi0", BCM2835_BLOCK_SIZE, memfd, BCM2835_SPI0_BASE) );
	    if (bcm2835_spi0 == MAP_FAILED) goto exit;

	    // I2C
	    bcm2835_bsc0 = static_cast<uint32_t *>( mapmem("bsc0", BCM2835_BLOCK_SIZE, memfd, BCM2835_BSC0_BASE) );
	    if (bcm2835_bsc0 == MAP_FAILED) goto exit;

	    bcm2835_bsc1 = static_cast<uint32_t *>( mapmem("bsc1", BCM2835_BLOCK_SIZE, memfd, BCM2835_BSC1_BASE) );
	    if (bcm2835_bsc1 == MAP_FAILED) goto exit;

	    // ST
	    bcm2835_st = static_cast<uint32_t *>( mapmem("st", BCM2835_BLOCK_SIZE, memfd, BCM2835_ST_BASE) );
